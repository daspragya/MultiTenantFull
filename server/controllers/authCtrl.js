...
authMiddleWare = async (req, res, next) => {
  const authHeader = req.headers.tenant;
  try {
    const tenantName = authHeader.split(" ")[1]; // Assuming "Bearer <token>" format
...
const switchTenant = async (req, res) => {
  try {
    ...
    const tenantId = await tenantIDFromTenantName(tenantName);
    auth.tenantId = tenantId;
    return res.status(200).json({ error: "Switch tenant sucessfully" });
  } catch (err) {
